...
const CARD_ORDER = [
  ...
  { type: 'community' as const, data: communityFiles[0] },
  { type: 'talk' as const, data: talks[0] },
  { type: 'experiment' as const, data: experiments[1] },
  { type: 'experiment' as const, data: experiments[2] },
  { type: 'community' as const, data: communityFiles[1] },
  { type: 'community' as const, data: communityFiles[2] },
  // { type: 'community' as const, data: talks[4] },
...
const renderEntry = (entry: (typeof CARD_ORDER)[number], rotate: number) => {
  ...
  return (
    <PhotoCard
      ...
      external
      rotate={rotate}
      media={<ImageWithSkeleton src={entry.data.url} alt={entry.data.name} loading="lazy" />}
    >
...
const CombinedMasonry: React.FC<{ title?: string }> = ({ title = 'Craft and Workshops' }) => {
